...
export function BottomNav() {
  ...
  return (
    <nav
      className="fixed inset-x-0 bottom-0 z-40 bg-paper-200 px-4 pt-2 dark:bg-paper-900"
      // Home-Indicator freihalten, auf Geräten ohne Notch bleibt 0.75rem
      style={{ paddingBottom: "max(0.75rem, env(safe-area-inset-bottom))" }}
    >
      <ul className="mx-auto flex max-w-3xl gap-1 rounded-card bg-paper-200 p-2 shadow-neu dark:bg-paper-900">
            ...
            <li key={href} className="flex-1">
              <Link
                ...
                className={`flex flex-col items-center gap-1 rounded-control px-1 py-2.5 text-[0.6875rem] font-semibold transition-all duration-200 ${
                  active
                    ? "bg-paper-200 text-accent shadow-neu-in-sm dark:bg-paper-900"
                    : "text-paper-700 active:shadow-neu-in-sm dark:text-paper-400"
                }`}
              >
